...
class ShipperServiceScreen extends StatefulWidget {
  const ShipperServiceScreen({super.key});
...
class _ShipperServiceScreenState extends State<ShipperServiceScreen> {
  ...
  String _convertServiceType(String value) {
  ...
  void _registerService() {
    if (imageFile != null) {
      context.read<ShipperServiceBloc>().add(
            OnAddNewShipperService(
              ShipperService(
                id: 0,
                content:
                    '${context.read<AuthorBloc>().currentUser!.phoneNumber} đăng kí ${_convertServiceType(_currentServiceType)}',
                shipperPhoneNumber:
                    context.read<AuthorBloc>().currentUser!.phoneNumber,
                status: ShipperServiceEnum.waiting.name,
                type: _currentServiceType,
                beginDate: DateTime.now(),
                endDate: getServiceEndDate(),
              ),
            ),
          );
    } else {
            ...
            children: [
                ...
                DropdownButton<String>(
                  ...
                  items: List<DropdownMenuItem<String>>.generate(
                    ...
                    (index) => DropdownMenuItem(
                      ...
                      child: Text(
                        _convertServiceType(_serviceTypeList[index]),
                      ),
                    ),
                  ),
              ...
              _fieldComponent(
                ...
                Row(
                  ...
                  children: [
                    Text(context.read<AppConfigBloc>().appConfig!.bankAccount),
                    _copyButton(
